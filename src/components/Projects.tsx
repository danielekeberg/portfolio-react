...
function Projects() {
    return (
        <div className="flex gap-6 overflow-x-auto snap-x snap-mandatory scrollbar-hide">
            {projects.map((p, i) => (
                <a href={p.live} target="_blank" key={i} className={`p-10 relative h-150 min-w-90 md:min-w-125 mb-10 ${i % 2 === 0 ? 'bg-neutral-800/50' : 'bg-neutral-700/50'} ${i % 2 === 0 ? 'hover:bg-neutral-700/50' : 'hover:bg-neutral-600/50'}`}>
